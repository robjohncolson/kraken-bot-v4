...
$recentDispatches = @(Get-RecentDispatchHistory -daysWindow 7)
Write-RunLog "injecting recent dispatch history ($($recentDispatches.Count) entries)"
$recentDispatchLines = @("## RECENT DISPATCH HISTORY (last 7 days, newest first)")
if ($recentDispatches.Count -gt 0) {
    foreach ($entry in $recentDispatches) {
        $recentDispatchLines += "- $($entry.ts) -- status=$($entry.status) action=$($entry.action) spec=$($entry.spec) commit=$($entry.commit) restarted=$($entry.restarted)"
    }
} else {
