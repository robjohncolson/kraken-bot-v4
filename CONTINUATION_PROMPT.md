...
7. **Backfill fidelity**: `scripts/backfill_shadow.py` now correctly filters out dry-run cycles and failed orders. **Fresh run confirms:** 59 reports scanned → 10 dry-runs dropped, 20 failed-orders dropped, 21 no-analyses, **8 real filled cycles** kept. Forward-return comparison on 6 evaluable cycles: **shadow wins 5/6 (83%), 1 tie, 0 live wins. Cumulative: live −11.59%, shadow −0.10%, shadow edge +11.49%.** This completely reverses the earlier Codex-05 analysis that showed shadow losing −6.42% — the difference was entirely the phantom RAVE/USD +35.25% that was counted as a live win but never actually filled (order rejected on ordermin). The narrow USD veto is strictly supportable now on real-fill evidence.
