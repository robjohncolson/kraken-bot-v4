...
class ClaudeSourceError(ValueError):
    """Base exception for Claude belief source configuration errors."""


class InvalidClaudeTimeoutError(ClaudeSourceError):
...
class InvalidClaudeLookbackError(ClaudeSourceError):
...
class ClaudeSource:
    """Adapter for forming a belief through the Claude Code CLI."""
    ...
    def __init__(
        ...
        *,
        ...
        if timeout_seconds < 1:
            raise InvalidClaudeTimeoutError(timeout_seconds)
        if last_n_closed_positions < 1:
            raise InvalidClaudeLookbackError(last_n_closed_positions)
    ...
    def _extract_payload(self, raw_output: str) -> Mapping[str, Any] | None:
        return self._extract_payload_object(self._decode_json_text(raw_output))
